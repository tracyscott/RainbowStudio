...
public abstract class UIInputBox2 extends UI2dComponent implements UIFocus {
  ...
  protected UIInputBox2() {
    this(0, 0, 0, 0);
  }

  protected UIInputBox2(float x, float y, float w, float h) {
    super(x, y, w, h);
  ...
  public UIInputBox2 setProgressColor(boolean hasProgressColor) {
    this.hasProgressColor = hasProgressColor;
    redraw();
    return this;
  }

  public UIInputBox2 setProgressColor(int progressColor) {
  ...
  public UIInputBox2 setProgressIndicator(ProgressIndicator meter) {
  ...
  public UIInputBox2 enableImmediateEdit(boolean immediateEdit) {
  ...
  public void setEditBuffer(String text) {
  ...
  public UIInputBox2 setEditable(boolean editable) {
  ...
  public UIInputBox2 setEnabled(boolean enabled) {
  ...
  protected void onDraw(UI ui, PGraphics pg) {
    ...
    String displayString = this.editing? this.editBuffer : getValueString(); //clipTextToWidth(pg, this.editing ? this.editBuffer : getValueString(), this.width - TEXT_MARGIN);

    if (this.textAlignHorizontal == PConstants.LEFT) {
      pg.textAlign(PConstants.LEFT, PConstants.TOP);
      pg.text(displayString, 2, 0); //this.height / 2);
    } else {
  ...
  protected void onKeyPressed(KeyEvent keyEvent, char keyChar, int keyCode) {
    if (this.editable) {
      if (this.editing) {
        ...
        } else if (keyCode == java.awt.event.KeyEvent.VK_CAPS_LOCK) {
          consumeKeyEvent();
          this.editing = false;
          redraw();
        }
